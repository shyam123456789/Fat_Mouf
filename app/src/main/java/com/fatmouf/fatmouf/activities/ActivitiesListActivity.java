...
public class ActivitiesListActivity extends MyAbstractActivity {
    ...
    public void initview() {
        ...
        adapter = new CompetitiveAdapter(this, publicList,"activitylist");
        recyclerView.setAdapter(adapter);

    }

    private void getMyChallengeList() {
        MyLog.LogE(TAG, "  getMyChallengeList  ");
        getProgressDialog().show();
        Call<HomePublicResponse> call = RetrofitService.RetrofitService().get_myChallenge_list("Bearer " + PreferenceManger.getPreferenceManger().getAuthToken());
    ...
    private void getAcceptedChallengeList() {
        MyLog.LogE(TAG, "  getAcceptedChallengeList  ");

        getProgressDialog().show();
        Call<HomePublicResponse> call = RetrofitService.RetrofitService().get_my_accepted_challenge_list("Bearer " + PreferenceManger.getPreferenceManger().getAuthToken());
        call.enqueue(new Callback<HomePublicResponse>() {
            @Override
            public void onResponse(Call<HomePublicResponse> call, Response<HomePublicResponse> response) {
                getProgressDialog().dismiss();
